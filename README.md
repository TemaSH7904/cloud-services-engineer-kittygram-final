...
- **VPC-сеть** `kittygram-network` с подсетью `kittygram-subnet` (`10.0.0.0/24`, зона `ru-central1-a`)
- **Группу безопасности** `kittygram-sg` — правила приведены в таблице ниже
...
| Направление | Протокол | Порты   | CIDR        | Описание       |
|-------------|----------|---------|-------------|----------------|
| Входящий    | TCP      | 22      | `0.0.0.0/0` | SSH            |
| Входящий    | TCP      | 9000    | `0.0.0.0/0` | Gateway (HTTP) |
| Исходящий   | Any      | 0–65535 | `0.0.0.0/0` | Весь трафик    |
