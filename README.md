...
**Админ-панель Django:** http://51.250.67.132:9000/admin/

| Поле   | Значение           |
|--------|--------------------|
| Логин  | `admin`            |
| Пароль | `ЗАМЕНИ_НА_СВОЙ`   |

> Учётная запись создана только для проверки проекта. Виртуальная машина удаляется (`terraform destroy`) после ревью.
